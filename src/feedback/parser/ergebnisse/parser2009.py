...
from feedback.models import Fragebogen2009
from feedback.parser.ergebnisse.parser import Parser


class Parser2009(Parser):
    @classmethod
    def create_fragebogen(cls, veranst, frageb):
        Fragebogen2009.objects.create(
            ...
            v_gesamt = cls.parse_int(frageb[25]),
            ue_wie_oft_besucht = cls.parse_int(frageb[26]),
            ue_besuch_ueberschneidung = cls.parse_boolean(frageb[27]),
            ue_besuch_qualitaet = cls.parse_boolean(frageb[28]),
            ue_besuch_verhaeltnisse = cls.parse_boolean(frageb[29]),
            ue_besuch_privat = cls.parse_boolean(frageb[30]),
            ue_besuch_sonstiges = cls.parse_boolean(frageb[31]),
            ue_a = cls.parse_int(frageb[33]),
            ue_b = cls.parse_int(frageb[34]),
            ue_c = cls.parse_int(frageb[35]),
            ue_d = cls.parse_int(frageb[36]),
            ue_e = cls.parse_int(frageb[37]),
            ue_f = cls.parse_int(frageb[38]),
            ue_g = cls.parse_int(frageb[39]),
            ue_h = cls.parse_int(frageb[40]),
            ue_i = cls.parse_int(frageb[41]),
            ue_i2 = cls.parse_niveau(frageb[42]),
            ue_j = cls.parse_int(frageb[43]),
            ue_k = cls.parse_int(frageb[44]),
            ue_gesamt = cls.parse_int(frageb[45]),
            zusaetzliche_zeit = cls.parse_int(frageb[46]),
            vorwissen_aussreichend = cls.parse_int(frageb[47]),
            empfehlung = cls.parse_int(frageb[48]),
        )
